class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.balance = self.price - self.funds
            if self.funds < self.price:
                return 'You must add ${} more funds.'.format(self.balance)
            else:
                if self.funds > self.price:
                    self.change = abs(self.balance)
                    self.stock -= 1
                    return 'Here is your {0} and ${1} change.'.format(self.name, self.change)
...
class Coin:
    ...
    def worth(self):
        ...
        if Mint.current_year - self.year > 50:
            self.extra = Mint.current_year - self.year - 50
            return self.cents + self.extra
        else:
            return self.cents
